...
# Import functions from main ETL DAG
...
def extract_historical_metrics(**context):
    ...
    for batch_num in range(total_batches):
        ...
        logging.info(f"Processing batch {batch_num + 1}/{total_batches} ({len(batch_dates)} days)")
        ...
        for project in projects:
            ...
            for metric_date in batch_dates:
                try:
                    # Always use historical API for backfill
                    metrics_data = fetch_project_metrics(
                        project_key, 
                        metric_date, 
                        config, 
                        use_history=True
        ...
        logging.info(f"Batch {batch_num + 1} complete. Total records so far: {len(all_metrics)}")
...
def load_historical_metrics(**context):
    """Load historical metrics into PostgreSQL with optimized batch processing"""
    raw_metrics = context['task_instance'].xcom_pull(task_ids='extract_historical_metrics', key='raw_metrics')
    projects = context['task_instance'].xcom_pull(task_ids='fetch_projects', key='projects')
                ...
                if metric_data.get('error') or not metric_data.get('metrics'):
                    ...
                    if last_known_dict:
                        ...
                        if metric_date not in existing_dates:
                            insert_carried_forward_metric(cursor, project_id, metric_date, last_known_dict)
                    ...
                    if metric_date in existing_dates:
                        ...
                        if result and result[0]:  # If existing is carried forward, update with real data
...
def validate_backfill(**context):
    ...
    if 'start_date' in dag_run_conf and 'end_date' in dag_run_conf:
        ...
        backfill_days = dag_run_conf.get('backfill_days', None)
        if backfill_days:
            start_date = (execution_date - timedelta(days=int(backfill_days))).strftime('%Y-%m-%d')
        else:
            start_date = '2025-01-01'
    ...
    try:
        ...
        for row in results:
            ...
            logging.info(f"  Average metrics: Bugs={row[6]}, Vulnerabilities={row[7]}, Code Smells={row[8]}")
            logging.info("")
...
fetch_projects_task >> extract_historical_metrics_task >> load_historical_metrics_task >> validate_backfill_task